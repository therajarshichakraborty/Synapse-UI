...
function SolanaIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 397.7 311.7" fill="currentColor">
      ...
      <path
        fill="url(#solana-gradient-1-reg)"
      ...
      />
      <path
        fill="url(#solana-gradient-2-reg)"
        d="M64.6,3.8C67.1,1.4,70.4,0,73.8,0h317.4c5.8,0,8.7,7,4.6,11.1l-62.7,62.7c-2.4,2.4-5.7,3.8-9.2,3.8H6.5c-5.8,0-8.7-7-4.6-11.1L64.6,3.8z"
      />
      <path
        fill="url(#solana-gradient-3-reg)"
...
export default function Card_03({ onSubmit, onSocialLogin, className }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await onSubmit?.({ name: name.trim(), email, password, acceptTerms });
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      {passwordRequirements.map((req) => (
                        <div key={req.label} className="flex items-center gap-1.5">
                          {req.met ? (
                            ...
                            <XCircle className="w-3 h-3 text-zinc-300 dark:text-zinc-600" />
                          )}
                          <span
                            className={cn(
                              "text-[11px]",
              ...
              <div className="space-y-1">
                <div className="flex items-start space-x-2">
                  ...
                  <Label
                    htmlFor="terms"
                    className="text-sm text-zinc-600 dark:text-zinc-400 font-normal cursor-pointer leading-tight"
                  >
                    I agree to the{" "}
                    <button
                      type="button"
